...
import { T369Model }            from '../../t369-inference/src/model.js';
...
class T369InferenceEngine {
  ...
  resetCache() {
    ...
    this.#speculative?.setKVCacheEnabled(true);
  }
...
export class SkyNode {
  ...
  async generateWithAI(request) {
    ...
    this.#totalRequests++;
    this.#recordAIChatMessage();
    if (resetCache) this.#engine.resetCache();

    const result = await this.#engine.generate(this.#buildPrompt(prompt, ai), {
      maxTokens, temperature, topP, useSpeculative,
    });

    this.#wisdomScore = Math.min(1.0, this.#wisdomScore + 0.0001);
    return { text: result.text, tokensGenerated: result.tokensGenerated, aiUsed: ai, source: result.source, wisdomScore: this.#wisdomScore };
  }
